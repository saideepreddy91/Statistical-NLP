...
if __name__ == "__main__":
    ...
    for dname in dnames:
        ...
        datas.append(data)
        model = learn_trigram(data, alpha, flag)
        models.append(model)
    ...
    for i in xrange(n):
        for j in xrange(n):
            perp_dev[i][j] = models[i].perplexity(datas[j].dev, alpha, 1)
            perp_test[i][j] = models[i].perplexity(datas[j].test, alpha, 1)
            perp_train[i][j] = models[i].perplexity(datas[j].train, alpha, 1)
